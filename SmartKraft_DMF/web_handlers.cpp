...
void WebInterface::startServer() {
    ...
    // GÜVENLİK: Eğer STA bağlantısı başarısızsa, AP'yi aç (erişim kaybını önle)
    if (!staConnected) {
        shouldStartAP = true;
        if (!hasStoredWiFi) {
            Serial.println(F("[WiFi] İlk kurulum - AP modu zorla açılıyor"));
        } else {
            Serial.println(F("[WiFi] STA başarısız - Erişim için AP modu açılıyor"));
        }
    }
    ...
    // WiFi güç yönetimini kapat (performans için)
    WiFi.setSleep(false);
...
void WebInterface::loop() {
    ...
    if (millis() - lastWiFiCheck > 30000) { // 30 saniyede bir kontrol
        ...
        if (mode == WIFI_STA || mode == WIFI_AP_STA) {
            ...
            else if (!wasConnected && !nowConnected && status == WL_IDLE_STATUS) {
                // WL_IDLE_STATUS = bağlanmaya çalışmıyor, sadece idle
                Serial.println(F("[WiFi] Bağlı değil, deneme yapılıyor..."));
                if (network) {
                    network->connectToKnown();
                }
            }
