...
#include "driver/temperature_sensor.h"  // Dahili sıcaklık sensörü
...
// ============================================
// DAHİLİ SICAKLIK SENSÖRÜ (ESP32-C6)
// ============================================
temperature_sensor_handle_t temp_sensor = NULL;
float currentTemperature = 0.0f;  // Son okunan sıcaklık
float maxTemperature = 0.0f;      // Görülen maksimum sıcaklık
float minTemperature = 100.0f;    // Görülen minimum sıcaklık

// Termal koruma durumları
enum ThermalState {
    THERMAL_NORMAL = 0,    // < 70°C - Normal çalışma
    THERMAL_WARNING = 1,   // 70-85°C - WiFi TX düşük
    THERMAL_CRITICAL = 2   // > 85°C - WiFi kapalı
};
ThermalState currentThermalState = THERMAL_NORMAL;
bool wifiDisabledByThermal = false;  // Termal koruma WiFi'yi kapattı mı?

String generateDeviceId() {
    // Format: "SmartKraft DMF ID:XXXXXXXXXXXX" (benzersiz 12 karakter)
    return "SmartKraft DMF ID:" + uniqueChipId;
}

String generateAPName() {
    // Format: "DMF-XXXXXXXXXXXX" (benzersiz 12 karakter)
    return "DMF-" + uniqueChipId;
}
...
// ============================================
// DAHİLİ SICAKLIK SENSÖRÜ FONKSİYONLARI
// ============================================
void initTemperatureSensor() {
    ...
    if (ret != ESP_OK) {
        LOG_CRITICAL("[TEMP] Sensör kurulum hatası: %d\n", ret);
        return;
    }
    
    ret = temperature_sensor_enable(temp_sensor);
    if (ret != ESP_OK) {
        LOG_CRITICAL("[TEMP] Sensör aktifleştirme hatası: %d\n", ret);
        return;
    }
    ...
    if (temperature_sensor_get_celsius(temp_sensor, &temp) == ESP_OK) {
        ...
        maxTemperature = temp;
        minTemperature = temp;
        LOG_INFO("[TEMP] Sensör OK: %.1f°C\n", temp);
    }
}

float readTemperature() {
    if (temp_sensor == NULL) return -999.0f;
    float temp = 0;
    if (temperature_sensor_get_celsius(temp_sensor, &temp) == ESP_OK) {
        currentTemperature = temp;
...
void checkThermalProtection() {
    ...
    if (temp >= 85.0f) newState = THERMAL_CRITICAL;
    else if (temp >= 70.0f) newState = THERMAL_WARNING;
    else if (temp < 65.0f) newState = THERMAL_NORMAL;
    else newState = currentThermalState;
    
    if (newState != currentThermalState) {
        LOG_CRITICAL("[THERMAL] Durum değişti: %s -> ", getThermalStateString());
        currentThermalState = newState;
        LOG_CRITICAL("%s (%.1f°C)\n", getThermalStateString(), temp);
        
        switch (newState) {
            case THERMAL_NORMAL:
                if (wifiDisabledByThermal) {
                    WiFi.mode(WIFI_STA);
                    networkManager.ensureConnected(false);
                    wifiDisabledByThermal = false;
                    LOG_CRITICAL("[THERMAL] WiFi yeniden aktif\n");
                }
                esp_wifi_set_max_tx_power(84);
                break;
            case THERMAL_WARNING:
                LOG_CRITICAL("[THERMAL] WiFi TX gücü düşürüldü\n");
                if (wifiDisabledByThermal) {
                    WiFi.mode(WIFI_STA);
                    networkManager.ensureConnected(false);
                    wifiDisabledByThermal = false;
            ...
            case THERMAL_CRITICAL:
                LOG_CRITICAL("[THERMAL] KRİTİK! WiFi kapatıldı!\n");
                WiFi.disconnect(true);
                WiFi.mode(WIFI_OFF);
                wifiDisabledByThermal = true;
                break;
        }
    }
}

const char* getThermalStateString() {
    switch (currentThermalState) {
        case THERMAL_NORMAL:   return "Normal";
        case THERMAL_WARNING:  return "Uyarı";
        case THERMAL_CRITICAL: return "Kritik";
...
void setup() {
    ...
    Serial.println(F("\n========================================"));
    Serial.println(F("   SmartKraft DMF - Starting..."));
    Serial.println(F("========================================"));
    ...
    LOG_CRITICAL("[BOOT] ✓ Firmware: %s | Heap: %lu | MaxBlok: %lu | Temp: %.1f°C\n", 
                 FIRMWARE_VERSION, ESP.getFreeHeap(), ESP.getMaxAllocHeap(), currentTemperature);
    Serial.println(F("========================================\n"));
}
...
void loop() {
    ...
    if (now - lastHealthLog > 300000) {
        ...
        LOG_CRITICAL("[STATUS] Temp: %.1f°C (Min:%.1f Max:%.1f) [%s]\n", 
                     temp, minTemperature, maxTemperature, getThermalStateString());
        ...
        if (!wifiOk && !apActive && !wifiDisabledByThermal) {
    ...
    if (now - lastWiFiReconnect > 30000) {
        if (!networkManager.isConnected() && !wifiDisabledByThermal) {
            wifi_mode_t mode = WiFi.getMode();
