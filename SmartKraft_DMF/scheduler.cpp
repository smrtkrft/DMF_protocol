...
uint32_t CountdownScheduler::totalDurationSeconds() const {
    if (currentSettings.unit == TimerSettings::DAYS) {
        return (uint32_t)currentSettings.totalValue * 24UL * 60UL * 60UL;
    }
    return (uint32_t)currentSettings.totalValue * 60UL * 60UL;
}
