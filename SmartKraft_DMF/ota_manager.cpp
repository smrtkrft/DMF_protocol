...
void OTAManager::begin(const String& firmwareVersion) {
    ...
    if (!state.startupCheckDone) {
        ...
        // Önceki oturumdan kalan süreyi kullan veya yeni hesapla
        if (state.nextCheckInterval == 0) {
            state.nextCheckInterval = calculateNextInterval();
        }
        state.lastCheckTime = bootTime;
        Serial.printf("[OTA] Sonraki kontrol: ~%.1f saat sonra\n", 
                      state.nextCheckInterval / 3600000.0);
    }
...
void OTAManager::loop() {
    ...
    if (elapsed >= state.nextCheckInterval) {
        // WiFi bağlı mı?
        if (wifiConnected) {
            Serial.println(F("[OTA] ⏰ Zamanlanmış OTA kontrolü başlatılıyor..."));
            ...
            // Sonraki aralığı hesapla (24-48 saat)
            state.nextCheckInterval = calculateNextInterval();
            state.lastCheckTime = now;
            ...
            // WiFi yok, 5 dakika sonra tekrar dene
            state.lastCheckTime = now;
            state.nextCheckInterval = 5UL * 60UL * 1000UL; // 5 dakika
            Serial.println(F("[OTA] WiFi bağlı değil, 5 dakika sonra tekrar denenecek"));
        }
    }
}
...
uint32_t OTAManager::calculateNextInterval() {
    // 24 saat + random(1 dakika - 24 saat)
    // Bu şekilde tüm cihazlar farklı zamanlarda kontrol eder (sunucu yükü dağılımı)
    
    uint32_t randomAdd = OTA_RANDOM_MIN_MS + (esp_random() % (OTA_RANDOM_MAX_MS - OTA_RANDOM_MIN_MS));
    uint32_t interval = OTA_BASE_INTERVAL_MS + randomAdd;
    
    Serial.printf("[OTA] Yeni aralık hesaplandı: %.1f saat (24h + %.1f saat random)\n",
                  interval / 3600000.0, randomAdd / 3600000.0);
